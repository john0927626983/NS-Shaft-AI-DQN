...
def network():
	...
	conv3 = Conv2D(filters=64, strides=1, activation='relu',padding='same',use_bias=True, 
		kernel_size=[3,3], kernel_initializer=initializer.TruncatedNormal(stddev=0.01),
	...
	opt = Adam(lr=0.0001)
	model.compile(optimizer=opt,loss=custom_loss(fc2,y))
    ...
    while 1:
        ...
        if B != 1:
            ...
            if t > OBSERVE:
                ...
                Qt.append(np.argmax(Q_t))
                timest.append(t)
                lossst.append(history.history['loss'][0])
                if t % 100 == 0:
                    plt.plot(list(timest),list(lossst),'r-')
                    #Q=plt.plot(list(timest),list(Qt),'b-')
                    plt.title('model loss')
                    plt.ylabel('Q')
                    plt.xlabel('timestep')
                    plt.legend(['loss', 'Q'], loc='upper left')
                    plt.show()
